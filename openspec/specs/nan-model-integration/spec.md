# nan-model-integration — Specification

## Purpose

Canonical capability contract for the `nan` provider integration in cortex-ia: static catalog metadata with monthly quotas and effort vocabularies, comment-preserving `model set --effort` variant auto-authoring in the OpenCode global config, read-only doctor validation of effort identifiers, the always-on TUI usage strip in `session.composer.top`, and the `:model` / `:nan` keymap layers. Requirements REQ-NAN-001..REQ-NAN-005 merged from change `nan-model-effort-usage` (board `nan-model-effort-usage`).
