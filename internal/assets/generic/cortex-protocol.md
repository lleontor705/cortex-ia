...
> **IMPORTANT**: Cortex and Engram refer to the same persistent memory system. The tools (`mem_save`, `mem_search`, etc.) are identical. "Cortex" is the current name; "Engram" is a legacy alias retained for backward compatibility with existing agent configurations.
