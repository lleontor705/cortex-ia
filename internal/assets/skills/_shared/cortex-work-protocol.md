...
Planner workflow/phase pairs are `decision-map` with `chart|resolve`, `sdd-lite` with `integrated|archive|decompose`, or `sdd-full` with `propose|spec|design|tasks|archive|decompose`. Other roles use the assigned operational phase (for example investigate/diagnose, direct-change/apply, or review/verify). The transport validates the versioned common fields and planning pairs before dispatch. Unversioned historical envelopes and one legacy `<minion-contract>` remain accepted for resume compatibility, with the existing identity and budget checks; supplied roles must match the host target. New prompts use the canonical tag.
