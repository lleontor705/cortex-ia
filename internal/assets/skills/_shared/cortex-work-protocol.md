...
Recovery only reconciles expired authority. It does not recreate claims or leases. Retry is explicit and uses a fresh attempt; never reuse tokens from an expired or terminal attempt. A task has an outer limit of five durable claim attempts, but MUST circuit-break earlier on review rejections: if a task receives two (2) consecutive review FAIL verdicts, or if the diff exceeds the workload budget (<= 500 LOC in Go, <= 350 LOC in TS/Python), the orchestrator MUST NOT retry the monolithic task directly. Implementers must check `git diff --stat` before calling `cortex_ia_work_transition({ to: "in_review" })`; if changed lines exceed the cap, transitioning to `in_review` is strictly forbidden and the task must transition directly to `blocked` with reason `WORKLOAD_BUDGET_EXCEEDED`. When timeout, scope, or 2 consecutive review failures show that the unit is overloaded or too large, the orchestrator routes the task to planner for atomic decomposition (`phase: "decompose"`). The planner designs 2-8 fully specified tasks (<= 250 LOC each) and invokes `cortex_ia_work_decompose` once instead of retrying the parent. Cortex applies that plan atomically: it preserves the board/project and upstream dependencies, chains the children, redirects downstream dependencies to the final child, and exposes the blocked parent as `superseded`. The orchestrator, implementers, and reviewers never invoke decomposition directly. Multi-record/batch validation must operate on defensive copies without mutating caller input in-place, and implementers must never weaken contracts to force tests green.
