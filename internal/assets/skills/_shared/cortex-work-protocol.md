...
Before choosing external execution, configure its scoped authentication explicitly: `CORTEX_IA_AGY_AUTH=gemini` and `GEMINI_API_KEY`. The runner generates only the documented Gemini provider setting in its temporary home; it does not copy user profiles, MCP configuration or keyring credentials. Non-Gemini model selections return `AGY_AUTH_MODEL_UNSUPPORTED` without replacing the chosen model; missing authentication returns `AGY_AUTH_REQUIRED`. The current scoped provider is Gemini only. This process environment boundary is not an OS, network or workspace sandbox. See the vendor's installation/authentication documentation when migrating provider configuration.
