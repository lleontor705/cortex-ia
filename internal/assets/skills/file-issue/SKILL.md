...
<rules>
<critical>
This skill implements the issue-first enforcement system. Every code change must start with an approved issue. Two templates are available: Bug Report and Feature Request. After creation, a maintainer must add `status:approved` before any PR can reference the issue.
...
6. Redirect questions to GitHub Discussions — use Discussions for questions, use issues only for bugs and features.
