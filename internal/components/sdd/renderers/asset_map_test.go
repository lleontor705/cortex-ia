...
func TestAdapterAssetMapMapsSemanticAssetsOnce(t *testing.T) {
	...
	if got.Scope != ir.ScopeWorkflowRoot || got.Relative != ".kiro/skills/implement/SKILL.md" {
...
func TestAdapterAssetMapPreservesKiroExternalRoot(t *testing.T) {
	m, err := AdapterAssetMapFor("kiro")
	if err != nil {
		t.Fatal(err)
	}
	if m.ExternalConfigRoot != "%APPDATA%/kiro/User" {
		t.Fatalf("external root = %q", m.ExternalConfigRoot)
	}
	if _, err := m.Map("asset/external", ir.AssetManifest, "%APPDATA%/kiro/User/settings.json"); err == nil {
		t.Fatal("external config was rebased into portable workflow map")
	}
}
