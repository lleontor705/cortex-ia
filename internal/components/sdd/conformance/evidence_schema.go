...
func ValidateCompleteEvidence(report EvidenceReport) error {
	...
	if len(report.Records) != 324 {
		return fmt.Errorf("complete evidence records = %d, want 324", len(report.Records))
	}
	cells := make(map[string]struct{}, 36)
	for _, record := range report.Records {
		cells[record.Cell] = struct{}{}
	}
	if len(cells) != 36 {
		return fmt.Errorf("complete evidence cells = %d, want 36", len(cells))
	}
	return nil
}
