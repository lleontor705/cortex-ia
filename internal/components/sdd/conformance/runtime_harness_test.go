...
func TestRuntimeHarnessProducesDeterministicReceiptLinkedMatrixEvidence(t *testing.T) {
	...
	if len(first.Bindings) != 324 || first.Fingerprint != second.Fingerprint {
		t.Fatalf("matrix evidence is not deterministic 36/324 output: bindings=%d fingerprints=%q/%q", len(first.Bindings), first.Fingerprint, second.Fingerprint)
	}
...
func TestRuntimeHarnessExecutesExactlyThirtySixProductionCells(t *testing.T) {
	...
	if got := len(receipt.Cells); got != 36 {
		t.Fatalf("executed cells = %d, want 36", got)
	}
	if got := len(receipt.Adapters) * len(receipt.Profiles); got != 36 {
		t.Fatalf("cartesian cardinality = %d, want 36", got)
	}
	...
	if len(seen) != 36 {
		t.Fatalf("unique cells = %d, want 36", len(seen))
	}
