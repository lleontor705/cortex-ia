...
func TestREQ_PROV_002_InstallWritesEveryCatalogModel(t *testing.T) {
	...
	if receipt.ModelsWritten != 8 || receipt.VariantsWritten != 20 || receipt.BackupID == "" || receipt.RollbackCmd != "cortex-ia rollback "+receipt.BackupID {
	...
	if got := strings.Join(providerVariantIDs(t, glm), ","); got != "low,medium,high,max" {
		t.Fatalf("glm5.3 variants = %s", got)
	}
	if deepseek, _ := models["deepseek-v4-flash"].(map[string]any); deepseek["variants"] != nil {
		t.Fatal("empty effort model must not carry variants")
	}

	record := state.LoadMetadataV2(home).Metadata.Providers[0]
	if record.Name != "nan" || record.ConfigPath != "opencode.jsonc" || len(record.Models) != 8 ||
...
func TestREQ_PROV_003_TokenStaysOutAndRotationKeepsDigest(t *testing.T) {
	...
	if rotatedReceipt.Action != "installed" || state.LoadMetadataV2(home).Metadata.Providers[0].SemanticDigest != digest ||
		options["apiKey"] != rotated || len(entry["models"].(map[string]any)) != 8 {
...
func TestREQ_PROV_002_PreviewIsInert(t *testing.T) {
	...
	if !receipt.DryRun || receipt.Action != "installed" || receipt.BackupID != "" ||
		receipt.ModelsWritten != 8 || receipt.VariantsWritten != 20 {
