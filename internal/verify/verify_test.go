...
func TestDoctorFindingsAreStableAndActionable(t *testing.T) {
	tests := []struct {
		...
		{name: "permission widened", observation: Observation{Kind: CheckPermissions, State: StateMismatch, Target: "cursor", Path: "permissions.network", Observed: "*", Expected: "api.example.test", Evidence: "security manifest", Remediation: "regenerate without permission widening"}, code: FindingPermissions, severity: SeverityError, blocking: true},
		{name: "secret rendered", observation: Observation{Kind: CheckSecrets, State: StatePresent, Target: "gemini", Path: "mcp.env.API_KEY", Observed: "literal secret", Expected: "opaque secret reference", Evidence: "redacted digest sha256:06", Remediation: "remove the value, rotate it, and regenerate"}, code: FindingSecret, severity: SeverityError, blocking: true},
