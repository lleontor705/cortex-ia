...
func runGGA(args []string) error {
	...
	// Every command processes exactly one flag and returns; there is no
	// multi-flag chaining, so args[0] is the sole dispatch target.
	switch args[0] {
	case "--list":
	...
	default:
		return fmt.Errorf("gga: unknown flag %q", args[0])
	}
}
