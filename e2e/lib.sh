#!/usr/bin/env bash
# lib.sh — shared test helpers for cortex-ia E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/cortex-ia inside
# the Docker container. If not found, fall back to $HOME/cortex-ia or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./cortex-ia in the current repo directory (freshly built local binary)
#   2. ~/cortex-ia (explicit copy in home)
#   3. cortex-ia on PATH (system-installed, e.g. Homebrew, Docker container)
# This ensures `go build ./cmd/cortex-ia && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    local repo_root
    repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
    if [ -x "$repo_root/cortex-ia" ]; then
        echo "$repo_root/cortex-ia"
    elif [ -x "./cortex-ia" ]; then
        echo "./cortex-ia"
    elif [ -x "$HOME/cortex-ia" ]; then
        echo "$HOME/cortex-ia"
    elif command -v cortex-ia >/dev/null 2>&1; then
        echo "cortex-ia"
    else
        echo ""
    fi
...
setup_fake_configs() {
    ...
    echo '{"fake": true}' > "$HOME/.claude/mcp/cortex.json"
}

# setup_fake_agent_binary NAME
# Creates a stub binary for an agent so cortex-ia detect picks it up.
setup_fake_agent_binary() {
    local name="$1"
    mkdir -p "$HOME/bin"
    printf '#!/bin/bash\n' > "$HOME/bin/$name"
...
# Checks that the cortex-ia section marker appears exactly once (no duplicates).
assert_no_duplicate_section() {
    ...
    local marker="<!-- cortex-ia:${section_id} -->"
    local count
